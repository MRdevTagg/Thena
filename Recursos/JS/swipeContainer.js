...
const containerAll = document.querySelectorAll('.swipecont')
const containParentAll = document.querySelectorAll('.swipecont-container')
const containParent = Array.from(containParentAll)
const container = Array.from(containerAll)
const $containers = [container[0],container[1],container[2],1,0,1]
...
function TouchendHderech(event) {
        ...
        if (Math.abs(move_x) > min_horizontal_move && Math.abs(move_y) < max_vertical_move && elapsed_time < within_ms) {
            if (move_x < 0 && puedeSwipe) {
                ...
                //alert("right");
                Derecha($containers,objReferencias,animRL2)
            }
        }
    }
...
containParent[0].addEventListener('touchstart', touch_start)
 containParent[0].addEventListener('touchmove', TouchendHderech)
